...
class Translator:
    ...
    def compute_wrench(self):
        ...
        for i in range(len(self.mot)):
            ...
            self.wrench.wrench.force.x += cmd[0]
            self.wrench.wrench.force.y += cmd[1]
            self.wrench.wrench.force.z += cmd[2]

        # torques
        for i in range(len(self.mot)):
            force = self.mot[i].for_dir*self.cmd[i]*self.mot[i].gain
            torque = np.cross(self.mot[i].pos, force)
            self.wrench.wrench.torque.x += torque[0]
            self.wrench.wrench.torque.y += torque[1]
            self.wrench.wrench.torque.z += torque[2]
